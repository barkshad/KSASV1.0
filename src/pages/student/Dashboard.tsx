...
export default function StudentDashboard() {
  ...
  React.useEffect(() => {
    ...
    // We can't do a cross-collection group query easily without indexes, 
    // but for this demo scale we can fetch sessions and then attendance,
    // or just fetch all archived from Cloudinary.
    // For now, let's fetch attendance from active sessions in firestore
  ...
  const overallAttendance = studentCourses.length > 0 ? 85 : 0; // Derived stat Placeholder - logic involves complex aggs
        ...
        <div className="bg-surface-container-lowest rounded-xl p-lg shadow-sm border border-outline-variant/30 flex items-center justify-between hover:shadow-md transition-shadow">
          ...
          <div className="relative h-20 w-20 flex items-center justify-center">
            <svg className="transform -rotate-90 w-20 h-20">
              ...
              <circle cx="40" cy="40" r="36" stroke="currentColor" strokeWidth="8" fill="transparent" strokeDasharray="226.2" strokeDashoffset={226.2 - ((overallAttendance/100) * 226.2)} className="text-tertiary-fixed-dim" strokeLinecap="round"></circle>
            </svg>
            ...
            <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-lg">
              <div>
                <div className="flex items-center gap-xs mb-sm">
                  ...
                  <span className="font-label-md uppercase tracking-wider text-primary-fixed-dim">{upcomingSession.date} • {upcomingSession.startTime} - {upcomingSession.endTime}</span>
                </div>
                <h4 className="font-headline-md text-on-primary mb-xs">{upcomingSession.courseName}</h4>
                <p className="font-body-lg text-primary-fixed-dim mb-md">{upcomingSession.courseCode}</p>
              ...
              <button 
                ...
                className="w-full md:w-auto mt-sm md:mt-0 bg-tertiary-fixed-dim hover:bg-tertiary-fixed text-on-tertiary-fixed font-bold py-3 px-6 rounded-lg shadow-md transition-all flex items-center justify-center gap-sm active:scale-95 group disabled:opacity-50 disabled:cursor-not-allowed"
              >
            ...
            {attendance.length > 0 ? (
                <ul className="divide-y divide-outline-variant/30">
                  ...
                  <li key={att.id} className="p-md hover:bg-surface-container-low transition-colors flex items-center justify-between">
                    ...
                    <span className={`font-label-md px-2 py-1 rounded-full ${att.status === 'present' ? 'bg-tertiary-fixed-dim/20 text-on-tertiary-container' : 'bg-error-container text-on-error-container'}`}>{att.status || 'Present'}</span>
                  </li>
                  ))}
                </ul>
            ) : (
