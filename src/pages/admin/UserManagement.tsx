...
import { Upload, Download, Search, Plus, Loader2, Users, UserCheck, Shield } from 'lucide-react';
    ...
    <>
      <div className="flex-1 flex flex-col px-4 md:px-6 py-6 max-w-7xl mx-auto w-full animate-in fade-in duration-500">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            <button className="flex items-center gap-2 px-4 py-2.5 bg-surface-container border border-outline-variant rounded-xl text-sm font-semibold text-on-surface hover:bg-surface-variant transition-colors">
        ...
        {/* Save result banner */}
        {saveResult && (
          <div className={`mb-4 p-3 rounded-xl text-sm font-medium ${saveResult.type === 'ok' ? 'bg-tertiary-fixed-dim/20 text-on-tertiary-container' : 'bg-error-container text-on-error-container'}`}>
            {saveResult.msg}
            ...
            {filteredUsers.map((u) => (
              ...
              >
                <div className="flex items-start justify-between">
                  ...
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full uppercase shrink-0 ${u.status === 'active' ? 'bg-tertiary-fixed-dim/20 text-on-tertiary-container' : 'bg-error-container text-on-error-container'}`}>
                    {u.status}
                  </span>
                </div>
      ...
      {showAddModal && createPortal(
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-surface rounded-3xl p-7 w-full max-w-md shadow-2xl border border-outline-variant/20 animate-in zoom-in-95 duration-200">
            <h3 className="font-bold text-lg text-on-surface mb-1">Create New Account</h3>
            ...
            <form onSubmit={handleManualSave} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full p-3 rounded-xl bg-surface-container border border-outline-variant/50 text-sm focus:outline-none focus:border-primary transition-all"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  placeholder="jane@kabarak.ac.ke"
                  className="w-full p-3 rounded-xl bg-surface-container border border-outline-variant/50 text-sm focus:outline-none focus:border-primary transition-all"
                />
              </div>
              <div>
                <label className="block text-xs font-bold text-on-surface-variant mb-1.5 uppercase tracking-wider">ID / Student Number</label>
                <input
                  required
                  value={newUser.uid || ''}
              ...
              <div>
                <label className="block text-xs font-bold text-on-surface-variant mb-1.5 uppercase tracking-wider">
                  Initial Password <span className="font-normal normal-case">(leave blank to auto-generate)</span>
                </label>
                <input
                  type="password"
                  value={newUser.plainPassword || ''}
                  onChange={(e) => setNewUser({ ...newUser, plainPassword: e.target.value })}
              ...
              <div className="flex gap-3 pt-2">
                <button
                  ...
                  className="flex-1 py-3 font-bold text-on-surface-variant hover:bg-surface-variant rounded-xl transition-colors text-sm"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  disabled={savingUser}
                  className="flex-1 py-3 bg-primary text-on-primary font-bold rounded-xl flex items-center justify-center gap-2 hover:bg-primary/90 transition-all disabled:opacity-60 text-sm"
                >
